...
export default function SearchBar({
  ...
  return (
    <div className="w-full relative z-20">
      <div className="glass-panel rounded-2xl p-2.5 transition-all shadow-xl shadow-indigo-950/20 border border-white/10 hover:border-indigo-500/40">
        <div className="flex items-center gap-3">
          ...
          <input
            ...
            placeholder={
              mode === 'semantic'
                ? 'Ask or search by concept (e.g. "Q3 roadmap meeting notes")...'
                : mode === 'hybrid'
                ? 'Hybrid vector + keyword search...'
                : 'Search notes by keyword...'
            }
            className="flex-1 bg-transparent border-none outline-none text-slate-100 placeholder-slate-400 text-sm md:text-base py-1"
          />

          {query && (
            <button
              onClick={() => setQuery('')}
              className="p-1 rounded-full text-slate-400 hover:text-white hover:bg-slate-800/60"
            >
              <X className="w-4 h-4" />
            </button>
          )}
          ...
          {/* Mode Switcher Tabs */}
          <div className="hidden sm:flex items-center bg-slate-900/80 p-1 rounded-xl border border-white/5 text-xs font-medium">
            <button
        ...
        {/* Filter Drawer */}
        {showFilters && (
          <div className="mt-3 pt-3 border-t border-white/10 flex flex-col gap-3 text-xs animate-in fade-in slide-in-from-top-1">
            ...
            <div className="text-[11px] text-slate-500 flex justify-between items-center pt-1">
              <span>HTTP Protocol: <span className="font-mono text-indigo-400">{methodUsed}</span> /api/notes/search</span>
              <span>Spec: IETF draft `QUERY` method with `POST` fallback</span>
            </div>
          </div>
        )}
      </div>
