      ...
      try {
        ...
        if (res.ok) {
          ...
          if (isMounted && Array.isArray(data.models)) {
            ...
            // If currently selected model is not set, initialize with default
  ...
  return (
    <div className={`relative ${className}`} ref={dropdownRef}>
      <button
        ...
        className="flex items-center gap-1.5 px-2.5 py-1 rounded-xl bg-slate-900/80 hover:bg-slate-800 border border-white/10 text-[11px] sm:text-xs text-slate-200 transition-all shadow-sm group"
        title="Select Groq LLM Model"
      >
        <Cpu className="w-3.5 h-3.5 text-indigo-400 shrink-0 group-hover:rotate-12 transition-transform" />
        <span className="font-medium truncate max-w-[110px] sm:max-w-[140px] text-left">
          {currentModel.name}
        </span>
        {currentModel.badge && (
          <span className="hidden sm:inline-block text-[9px] px-1 py-0.2 rounded bg-indigo-500/20 text-indigo-300 font-mono border border-indigo-500/30 shrink-0">
            {currentModel.badge}
          </span>
        )}
      ...
      {isOpen && (
        <div className={`absolute ${alignClass} top-full mt-2 w-72 sm:w-84 rounded-2xl bg-[#0b0f19] border border-white/20 shadow-[0_20px_50px_rgba(0,0,0,0.9)] p-2 z-[100] animate-in fade-in slide-in-from-top-1 text-xs`}>
            ...
            {models.map((model) => {
              ...
              return (
                ...
                >
                  ...
                  {isSelected && (
                    <div className="p-1 rounded-lg bg-indigo-500/30 text-indigo-300 shrink-0 mt-0.5">
